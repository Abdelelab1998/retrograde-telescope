...
export default function App() {
  ...
  return (
    <div className={`fixed inset-0 w-full h-full overflow-hidden antialiased ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-gray-100 text-gray-900'}`} style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div className={`absolute ${isMobile ? 'top-4 left-4 right-4' : 'top-6 left-6'} z-20 flex flex-col gap-3 ${isMobile ? 'w-auto' : 'max-w-md w-full'} pointer-events-none`}>
        <div className={`rounded-lg p-1 flex flex-col shadow-xl border pointer-events-auto overflow-hidden backdrop-blur-xl ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center gap-3 px-4 py-2.5">
            <Search className={`w-4 h-4 ${theme === 'dark' ? 'text-white/40' : 'text-gray-400'}`} />
            <input
              type="text"
              placeholder="Search flights or airports..."
              className={`bg-transparent border-none outline-none text-sm w-full font-medium ${theme === 'dark' ? 'placeholder:text-white/30 text-white' : 'placeholder:text-gray-400 text-gray-900'}`}
              value={searchQuery}
            ...
            {searchQuery && (
              <button
                ...
                className={`p-1 rounded transition-colors ${theme === 'dark' ? 'hover:bg-white/10 text-white/40 hover:text-white' : 'hover:bg-gray-100 text-gray-400 hover:text-gray-900'}`}
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          {(searchResults.flights.length > 0 || searchResults.airports.length > 0) && (
            <div className={`border-t max-h-[60vh] overflow-y-auto ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
              <div className="p-1.5 space-y-2">
                {/* Flights */}
                {searchResults.flights.length > 0 && (
                  <div>
                    <div className={`px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-white/40' : 'text-gray-500'}`}>Flights</div>
                    <div className="space-y-0.5">
                      {searchResults.flights.map((flight) => (
                        <button
                          key={flight.icao24}
                          onClick={() => handleFlightClick(flight)}
                          className={`w-full flex items-center justify-between px-2.5 py-2.5 rounded transition-colors ${theme === 'dark' ? 'hover:bg-white/10' : 'hover:bg-gray-100'}`}
                        >
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded flex items-center justify-center border ${theme === 'dark' ? 'bg-white/10 border-white/20' : 'bg-gray-100 border-gray-300'}`}>
                              <Plane className={`w-3.5 h-3.5 ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`} />
                            </div>
                            <div className="text-left">
                              <div className={`text-xs font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{flight.callsign}</div>
                              <div className={`text-[10px] ${theme === 'dark' ? 'text-white/50' : 'text-gray-500'}`}>{flight.airline}</div>
                            </div>
                          </div>
                          <div className={`text-[10px] font-mono ${theme === 'dark' ? 'text-white/60' : 'text-gray-600'}`}>{flight.origin} → {flight.destination}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {searchResults.airports.length > 0 && (
                  <div>
                    <div className={`px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-white/40' : 'text-gray-500'}`}>Airports</div>
                    <div className="space-y-0.5">
                      {searchResults.airports.map((airport) => (
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded flex items-center justify-center border ${theme === 'dark' ? 'bg-white/10 border-white/20' : 'bg-gray-100 border-gray-300'}`}>
                              <MapPin className={`w-3.5 h-3.5 ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`} />
                            </div>
                            <div className="text-left">
                              <div className={`text-xs font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{airport.name}</div>
                              <div className={`text-[10px] ${theme === 'dark' ? 'text-white/50' : 'text-gray-500'}`}>{airport.city}, {airport.country}</div>
                            </div>
                          </div>
                          <div className={`text-[11px] font-bold ${theme === 'dark' ? 'text-white/60' : 'text-gray-600'}`}>{airport.iata || airport.icao}</div>
                        </button>
                      ))}
                    </div>
                  </div>
        ...
        {!isMobile && (
          <div className="flex items-center gap-2 px-0.5">
            <div className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2 border shadow-lg backdrop-blur-xl ${theme === 'dark' ? 'border-white/10 bg-[#1a1a1a]' : 'border-gray-200 bg-white'}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${theme === 'dark' ? 'bg-white' : 'bg-gray-900'}`}></div>
              <span className={`font-mono ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{flights.length}</span>
              <span className={`${theme === 'dark' ? 'text-white/50' : 'text-gray-500'}`}>Flights</span>
            </div>
            {error && (
              <div className={`px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 border backdrop-blur-xl ${theme === 'dark' ? 'border-white/20 text-white/60 bg-[#1a1a1a]' : 'border-gray-200 text-gray-600 bg-white'}`}>
      ...
      <div className={`absolute ${isMobile ? 'bottom-4 left-4' : 'top-6 right-6'} z-20 pointer-events-none`}>
        <div className="flex flex-col gap-2 pointer-events-auto">
          ...
          <button
            onClick={toggleTheme}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg transition-colors border shadow-xl backdrop-blur-xl ${theme === 'dark' ? 'bg-[#1a1a1a] hover:bg-[#252525] border-white/10' : 'bg-white hover:bg-gray-50 border-gray-200'}`}
          >
            {theme === 'dark' ? (
          ...
          >
            ...
            <div className={`w-10 h-5 rounded-full transition-colors ${useDummyData ? 'bg-amber-500/30' : 'bg-emerald-500/30'}`}>
              <div className={`w-4 h-4 rounded-full shadow-lg transition-transform ${useDummyData ? 'translate-x-5' : 'translate-x-0.5'} mt-0.5 ${theme === 'dark' ? 'bg-white' : 'bg-gray-900'}`} />
            </div>
          </button>
        </div>
      </div>
                ...
                <div
                  className={`transition-all duration-200 cursor-pointer ${
                    selectedFlightId === flight.icao24
                      ? theme === 'dark' ? 'text-white scale-125' : 'text-gray-900 scale-125'
                      : theme === 'dark' ? 'text-white/60 hover:text-white hover:scale-110' : 'text-gray-700 hover:text-gray-900 hover:scale-110'
                    }`}
                >
      ...
      {selectedFlight && isMobile && isMobilePanelOpen && (
        <div className={`absolute bottom-0 left-0 right-0 z-30 max-h-[70vh] flex flex-col rounded-t-2xl shadow-2xl border-t backdrop-blur-xl ${theme === 'dark' ? 'border-white/10 bg-[#1a1a1a]' : 'border-gray-200 bg-white'}`}>
          <div className={`flex items-center justify-between p-4 border-b ${theme === 'dark' ? 'border-white/10' : 'border-gray-200'}`}>
            <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{selectedFlight.callsign}</h3>
            <button
              onClick={() => {
                setIsMobilePanelOpen(false);
                setSelectedFlightId(null);
              }}
              className={`p-2 rounded transition-colors ${theme === 'dark' ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-900'}`}
            >
              <X className="w-5 h-5" />
            </button>
          </div>
